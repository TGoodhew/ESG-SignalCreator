...
namespace EsgSignalCreator.Tests.Assistant
{
    public class ConfigureToolsTests
    {
        ...
        [Fact]
        public async Task Configure_digital_video_routes_args_to_host_with_area()
        {
            var host = new FakeConfigureHost();
            await Run("configure_digital_video", new JObject { ["mode"] = "Mode8K" }, host);
            Assert.Equal("digital_video", host.LastConfigureArea);
        }

        [Fact]
        public async Task Configure_tdmb_routes_args_to_host_with_area()
        {
            var host = new FakeConfigureHost();
            await Run("configure_tdmb", new JObject { ["mode"] = "ModeI" }, host);
            Assert.Equal("t_dmb", host.LastConfigureArea);
        }

        [Fact]
        public async Task Configure_wimax_mobile_routes_args_to_host_with_area()
        {
            var host = new FakeConfigureHost();
            await Run("configure_wimax_mobile", new JObject { ["fft_size"] = "Fft1024" }, host);
            Assert.Equal("wimax_mobile", host.LastConfigureArea);
        }
