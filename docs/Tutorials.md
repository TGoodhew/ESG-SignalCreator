...
**Part I — Signal personality reference**
- [Signal personality reference (standards, broadcast & utility)](#part-i--signal-personality-reference)
...
Parts A–H walk through the fundamentals hands-on. This part is a **reference appendix** for the
standards, broadcast and utility personalities that don't have a dedicated step-by-step tutorial:
each entry says what the signal is, its key parameters, its **v2** option, and how to build it, with
an **app-rendered spectrum** so you know what to expect on screen before you touch hardware.

**Scope & how to read it.** These entries are deliberately concise — the build recipe is always the
same shape you learned in Part C (**Source → pick the personality → set parameters → Calculate →
inspect the plot views**), so we don't repeat every click. The spectra below are **offline app
renders** (deterministic, generated by `ESG-SignalCreator.exe --tutorial-images`, so they regenerate
identically). The matching **real analyzer captures** (spectrum / CCDF on an N9010A or E4406A) are a
separate bench task and are tracked in issue #230; they are not required to follow this reference.

**The six fundamentals already have full tutorials** — start there for the basics:

| Personality | Tutorial |
|---|---|
| CW / single tone | [Tutorial 1](#tutorial-1--take-the-tour--build-your-first-cw-tone-offline) / [2](#tutorial-2--connect-to-the-esg-and-play-a-cw-tone) |
| Multitone | [Tutorial 3](#tutorial-3--multitone-and-papr) |
| AWGN | [Tutorial 4](#tutorial-4--band-limited-noise-awgn) |
| Custom digital modulation | [Tutorial 5](#tutorial-5--custom-digital-modulation-qpsk--rrc) |
| Multi-Carrier | [Tutorial 6](#tutorial-6--multi-carrier-signals) |
| Import I/Q | [Tutorial 7](#tutorial-7--import-an-iq-file) |

Everything below is reachable the same way: **Source**, pick the personality in the picker, set the
parameters, **Calculate**, then read the **Spectrum**/**CCDF**/**Constellation** views (see UserGuide
§5, *Signal personalities*, for the authoritative parameter reference).

### I.1 — Cellular CDMA (multi-code composites)

All four spread data symbols by an OVSF code, complex-scramble, and root-raised-cosine shape at the
chip rate. Their **v2** addition is a **multi-code composite** — set **code channel count > 1** to sum
several orthogonal code channels (a more realistic downlink) instead of a single code.

**3GPP W-CDMA FDD (`wcdma-fdd`)** — 3.84 Mcps, SF 16, QPSK. v2: `code_channel_count = 4`.
Build: Source → **3GPP W-CDMA FDD** → spreading factor 16, code channels 4 → Calculate → Spectrum.

![W-CDMA FDD 4-code multiplex — spectrum](images/tutorials/ref-wcdma-fdd-spectrum.png)

**3GPP W-CDMA HSPA (`wcdma-hspa`)** — W-CDMA with higher-order modulation (QAM16 default). v2:
`code_channel_count = 4`.

![HSPA 4-code multiplex — spectrum](images/tutorials/ref-wcdma-hspa-spectrum.png)

**3GPP2 cdma2000 (`cdma2000`)** — 1.2288 Mcps, Walsh-coded, QPSK. v2: `code_channel_count = 4`.

![cdma2000 4-code multiplex — spectrum](images/tutorials/ref-cdma2000-spectrum.png)

**TD-SCDMA (`td-scdma`)** — 1.28 Mcps low-chip-rate TDD CDMA. v2: `code_channel_count = 4`.

![TD-SCDMA 4-code multiplex — spectrum](images/tutorials/ref-td-scdma-spectrum.png)

### I.2 — Cellular OFDM (LTE)

E-UTRA OFDM. The **v2** addition is a **frame-structured** mode (`frame_structured = true`) that builds
a real subframe/frame grid with the correct cyclic prefix and physical-cell-id, instead of a generic
OFDM block.

**3GPP LTE FDD (`lte-fdd`)** — bandwidth (1.4–20 MHz), modulation, normal/extended CP, subframe count.
v2 built here at 5 MHz, QAM16, normal CP.

![LTE FDD 5 MHz E-UTRA frame — spectrum](images/tutorials/ref-lte-fdd-spectrum.png)

**3GPP LTE TDD (`lte-tdd`)** — adds the TDD uplink/downlink configuration (0–6) and special-subframe
configuration (0–9). v2 built here at 5 MHz, UL/DL config 1, special-subframe config 7.

![LTE TDD 5 MHz frame — spectrum](images/tutorials/ref-lte-tdd-spectrum.png)

### I.3 — Wireless LAN & WiMAX

OFDM/OFDMA. **v2** adds a **frame-structured** PPDU/frame with the preamble/LTF and the standard's
cyclic-prefix / guard-interval choice.

**802.11 WLAN (`wlan-80211`)** — 20/40 MHz OFDM. v2 (20 MHz): `frame_structured = true`, long guard
interval, LTF preamble.

![802.11 20 MHz PPDU — spectrum](images/tutorials/ref-wlan-80211-spectrum.png)

**802.16-2004 Fixed WiMAX (`wimax-fixed`)** — channel-bandwidth-driven OFDM. v2: framed with a
preamble and a cyclic-prefix ratio (¼…1⁄32).

![Fixed WiMAX 3.5 MHz frame — spectrum](images/tutorials/ref-wimax-fixed-spectrum.png)

**802.16e Mobile WiMAX (`wimax-mobile`)** — scalable OFDMA (128–2048-FFT). v2: framed with preamble +
cyclic-prefix ratio.

![Mobile WiMAX 512-FFT OFDMA frame — spectrum](images/tutorials/ref-wimax-mobile-spectrum.png)

### I.4 — Digital broadcast (COFDM)

DAB/DVB-family COFDM. **v2** adds the **frame structure** — sync/phase-reference symbols (T-DMB) and
scattered pilots (DVB-T).

**T-DMB (`t-dmb`)** — DAB COFDM (transmission Modes I–IV), DQPSK. v2: `frame_structured = true` (adds
the null + phase-reference sync symbols). Shown here in Mode III.

![T-DMB Mode III COFDM frame — spectrum](images/tutorials/ref-t-dmb-spectrum.png)

**Digital Video / DVB-T (`digital-video`)** — 2K/8K COFDM with a guard interval. v2:
`frame_structured = true` inserts the scattered pilots. Shown here in 2K.

![DVB-T 2K COFDM with scattered pilots — spectrum](images/tutorials/ref-digital-video-spectrum.png)

### I.5 — Short-range & cellular narrowband

Constant-envelope defaults with a higher-order **v2** mode.

**GSM/EDGE (`gsm-edge`)** — GMSK by default; **v2** selects **EDGE 3π/8-rotated 8-PSK**
(`modulation = Edge8Psk`) for the EDGE linear-modulation path.

![EDGE 8-PSK — spectrum](images/tutorials/ref-gsm-edge-spectrum.png)

**Bluetooth (`bluetooth`)** — GFSK by default; **v2** selects **Enhanced Data Rate** —
`Edr2Mbps` (π/4-DQPSK) or `Edr3Mbps` (8-DPSK). Shown here at EDR 3 Mb/s.

![Bluetooth EDR 8-DPSK — spectrum](images/tutorials/ref-bluetooth-spectrum.png)

### I.6 — Analog broadcast

**Broadcast Radio / FM (`broadcast-radio`)** — stereo FM multiplex (mono + 38 kHz stereo subcarrier +
19 kHz pilot). **v2** adds the **RDS** 57 kHz biphase subcarrier (`rds = true`, `rds_deviation_hz`).

![Stereo FM with RDS subcarrier — spectrum](images/tutorials/ref-broadcast-radio-spectrum.png)

### I.7 — Test & impairment utilities

**Multitone Distortion (`multitone-distortion`)** — a dense equal-tone comb for IMD/NPR testing.
**v2** adds per-tone magnitude/phase tables, pre-distortion, and an NPR notch. Shown with a notch.

![16-tone comb with an NPR notch — spectrum](images/tutorials/ref-multitone-distortion-spectrum.png)

**Pulse (`pulse`)** — a pulse train with configurable width/PRI. **v2** adds intra-pulse modulation
(LFM chirp, Barker/Frank/polyphase codes) and PRI patterning (staggered/jittered). Shown as an LFM
chirp.

![1 µs LFM chirp, 10 µs PRI — spectrum](images/tutorials/ref-pulse-spectrum.png)

**Jitter (`jitter`)** — a jittered clock for serial-link stress. **v2** adds an SJ frequency sweep and
standards jitter masks (with a custom-mask option). Shown as sinusoidal SJ on a 10 MHz clock.

![10 MHz clock, sinusoidal SJ — spectrum](images/tutorials/ref-jitter-spectrum.png)

> 🧭 **Coverage note.** Every registered personality has an entry here or a full tutorial in Parts
> A–H — the reference images are generated by iterating the app's verification battery, so a newly
> added personality is included automatically (and the battery is guarded by a test, so it can't
> silently drop out). What is intentionally *not* here is a full click-by-click walkthrough per
> standard; the build recipe is identical to Part C. Real analyzer captures for these signals are the
> bench task tracked in issue #230.
