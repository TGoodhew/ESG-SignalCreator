...
### 5.7 Import I/Q
Indlæs I/Q fra en fil — appens svar på Signal Studio Toolkit (N7622A) "medbring din egen I/Q"-arbejdsgang. Parametre: **filsti** (du angiver den), **format** (**Auto**-detektér efter filendelse, afgrænset tekst CSV/TSV, rå interleaved little-endian **Int16** `.bin`/`.iq`, Agilent/Keysight big-endian **Int16** `.agt` — ESG'ens native ARB-byteorden — Agilent/Keysight big-endian **14-bit** (`AgilentInt14Be`; 14-bit-samplen er venstrejusteret i hvert 16-bit-ord, de 2 laveste bit er markører), 16-bit PCM **WAV** eller **MATLAB Level-5** `.mat`), kilde-**samplingsrate** (Hz), valgfri **I/Q-swap** og en **skala**-multiplikator. Angiv formatet eksplicit for at gennemtvinge en byteorden, når en filendelse er tvetydig. Lader dig genafspille eksternt opfangede eller eksternt genererede (MATLAB/C++) signaler.

**MATLAB `.mat` (MAT File 5)** — både ukomprimerede (`save -v6`) og de standard-zlib-komprimerede (v7) filer læses. Det første egnede numeriske array bruges: en **kompleks** vektor bliver til I/Q, et reelt **2×N- eller N×2**-array bliver til I og Q via de to rækker/kolonner, og en reel vektor bliver til I (Q = 0).

**Markør-/trigger-forfatning** — vedhæft en markør-bitstrøm til det importerede ARB-segment: **None**, **Start** (én markør ved sample 0 — en segment-trigger), **Periodic** (en markør hver *periode* samples) eller **Range** (en markørblok på *længde* samples fra *start*). Markørerne driver ESG'ens EVENT-/markørudgang ved afspilning.

### 5.8 Multitone Distortion
Et tæt multitone-/noise-power-ratio-(NPR)-stimulus til linearitetstest af forstærkere og konvertere (Signal Studio for Multitone Distortion, N7621B). Parametre: **antal toner** (2 op til 4097), **toneafstand** (Hz), **centeroffset** (Hz), en **fase-preset** — **Parabolic** (Newman, lav PAPR), **Random** eller **Constant** (justeret, høj PAPR) — og en valgfri **NPR-notch** (aktivér, **bredde** Hz, **offset** Hz fra båndcentrum), der rydder et bånd af toner, så du kan måle intermodulation/støj, der falder ned i notchen. Sammensat båndbredde ≈ antal toner × afstand; live-PAPR rapporteres.

**Per-tone-tabeller.** Valgfrie tabeller for **per-tone-magnitude** (dB) og **per-tone-fase** (grader) cykler hen over kammen (tone *k* bruger element *k* mod længde) og tilsidesætter den ensartede effekt og fase-preset'en — brug dem til at forme kammen eller finjustere crest-faktoren. **Prædistortionskorrektion** (aktivér) tager tabeller med **målt per-tone-magnitude-/fasefejl** (fra en signalanalysator, f.eks. via den lukkede Verify-sløjfe) og trækker dem fra basisværdierne pr. tone, så den målte kanalrespons forudinverteres, og den udsendte kam lander fladt, og IMD i NPR-notchen undertrykkes. Uden-for-bånd-notch-annullering (indsprøjtning af korrektionstoner inde i notchen) er endnu ikke implementeret.

### 5.9 Jitter Injection
Et jittret clock-/tonesignal til jitter-tolerancetest af modtagere (Signal Studio for Jitter Injection, E4438C-SP1). Jitter påføres som timing-(fase)-modulation af en sinusformet clock, så envelopen forbliver konstant. Parametre: **clock-rate** (Hz, signalet der jittres); **periodisk jitter**-form (**Sinusoidal / Square / Triangle / SawTooth / Exponential / Custom** eller None), **rate** (Hz) og **amplitude** (UI peak-to-peak); og valgfri **random** gaussisk jitter (**RMS** i UI + **seed**). Periodisk + random kombineres til et **composite**-signal, og en given seed reproducerer en identisk sekvens (digital reproducerbarhed). Amplituder angives i unit intervals (1 UI = én clock-periode).
...
- **EDGE 8-PSK** (v2, #186) — **3π/8-kontinuerligt-roteret 8-PSK** (3 bit/symbol), pulsformet og peak-normaliseret (ikke-konstant envelope, i modsætning til GMSK). Pulsen er en repræsentativ RRC-erstatning for den lineariserede GMSK-puls.
...
**Multikode-komposit** (v2, #183): sæt **code channel count** > 1 for at summere så mange ortogonale OVSF-kodekanaler (koder 0…N-1, samme effekt) til et multikode-downlink — kompositten scrambles og formes som ét signal (hvilket hæver PAPR). Stadig repræsentativt — de navngivne fysiske kanaler (CPICH/P-CCPCH/SCH), slot-/frame-struktur, TFCI, transmit diversity og HSPA-kanaler er endnu ikke implementeret.

### 5.13 3GPP W-CDMA HSPA
Et HS-PDSCH-lignende HSPA-signal (Signal Studio for 3GPP W-CDMA HSPA, E4438C-419). Samme DSSS-struktur som §5.12, men med **16QAM** på en **SF-16**-kode som standard — HSPA's definerende træk er højere-ordens modulation på den højhastigheds-delte kanal. Parametre svarer til W-CDMA FDD plus et **modulations**-valg (QPSK / 16QAM / 64QAM). **Multikode** (v2, #184): sæt **code channel count** > 1 for at summere flere **HS-PDSCH-multikoder** (som HSDPA gør) til kompositten. Repræsentativt — HS-SCCH/HS-DPCCH, E-DCH-kanaler, H-ARQ og TTI-struktur er endnu ikke implementeret.

### 5.14 3GPP2 CDMA (cdma2000)
Et cdma2000-forward-link-lignende signal (Signal Studio for 3GPP2 CDMA, N7601B). Samme DSSS-struktur som §5.12 ved **1,2288 Mcps** chip-raten: QPSK-data spredt med en **Walsh**-kode, PN-scramblet, pulsformet (RRC-tilnærmelse af cdma2000-filteret). Parametre svarer til W-CDMA FDD. **Multikode** (v2, #185): sæt **code channel count** > 1 for at summere flere **Walsh-kodekanaler** til et flerkanals forward-link-komposit. Repræsentativt — pilot-/sync-/paging-kanaler, radiokonfigurationer og frame-/PCG-struktur er endnu ikke implementeret, og det eksakte cdma2000-basebåndsfilter tilnærmes af RRC.
...
- **Rammestruktureret** (v2, #188) — aktivér **frame structured** for at bygge en rigtig **E-UTRA downlink-radioramme**: en 10 ms-ramme af 0,5 ms-slots med **per-symbol cyklisk præfiks** (**Normal** = 7 symboler/slot eller **Extended** = 6), **PSS** (Zadoff-Chu) og **SSS** synkroniseringssignaler på deres standardsymboler (de centrale 62 subcarriers i slot 0 og 10), **CRS** celle-specifikke referencesignaler (antenneport 0) på standardpositionerne og en **PDSCH**-datafyldning på resten — alt styret af **physical cell ID** (0…503). Sæt **subframe count** (10 = én radioramme).
...
- **Rammestruktureret** (v2, #195) — aktivér **frame structured** for en DAB-transmissionsramme: **synkroniseringskanalen** (et **null-symbol** af stilhed til rammesynkronisering og derefter et **fasereferencesymbol** med en kendt fase pr. bærebølge) efterfulgt af **differentielt kodede DQPSK**-datasymboler (hver bærebølges fase akkumulerer QPSK-data-deltaet fra det foregående symbol).
